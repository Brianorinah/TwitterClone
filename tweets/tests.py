...
class TweetTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="brian", password="shaktar")
        self.user2 = User.objects.create_user(
            username="Shakur", password="shakur")
        Tweet.objects.create(content="First tweet", user=self.user)
        Tweet.objects.create(content="Second tweet", user=self.user)
        Tweet.objects.create(content="Third tweet", user=self.user2)
        Tweet.objects.create(content="Fourth tweet", user=self.user2)
        self.current_count = Tweet.objects.all().count()

    def test_tweet_created(self):
        tweet_obj = Tweet.objects.create(content="Fifth tweet", user=self.user)
        self.assertEqual(tweet_obj.id, 5)
        self.assertEqual(tweet_obj.user, self.user)

    def get_client(self):
        client = APIClient()
        client.login(username=self.user.username, password='shaktar')
